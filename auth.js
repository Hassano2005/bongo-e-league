...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({
            ...
            token,
            user: { id: newUser.id, username, role: 'player' }
        });
    } catch (err) {
        console.error(err);
...
// ✅ EXPORT authenticate middleware for protected routes (profile, admin, tournaments)
const authenticate = (req, res, next) => {
    ...
    try {
        ...
        req.userId = decoded.id; // Match the token payload structure
        next();
    } catch (err) {
